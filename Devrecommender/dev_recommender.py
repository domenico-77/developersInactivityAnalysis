...
class Dev_recomender:
    ...
    def __save_devs_login_list(self):
        ...
        if not exist:
            ...
            devs_login_list = set(self.__get_devs_login_list())
            devs_login_commit = self.__get_devs_login_by_commit_list()
            devs_login_list = devs_login_list | devs_login_commit
            devs_login_list = list(devs_login_list)
    ...
    def __get_devs_login_by_commit_list(self):
        commit_list = self.__read_commit_list()
        login_devs = set()
        for commit in commit_list:
            login_devs.add(commit['author'])
    ...
    def cosine_similarity(self, first_dev, second_dev,decimal_places):
        ...
        if scalar_product == 1:
            ...
            cosine_similarity = round(cosine_similarity, decimal_places)
        return cosine_similarity
    
    def recomender(self, dev_stats, devs_stats, k, decimal_places):
        """
    ...
    def save_dev_s_replacements(self, dev_gone, classification):
        path_replacement = get_path_to_dev_replacements(str(dev_gone['dev']))
        ...
        with open(path_replacement, mode= 'w', newline='') as file:
            if classification:
                ...
                writer.writerow(["There is no data needed to find replacements for the developer" + str(dev_gone['dev'])])
        
    def recomender_substitutes(self, k, decimal_places):
        stats_devs_gone = self.get_stats_devs_gone()
        devs_stats = self.update_devs_stats(stats_devs_gone)
        for dev_gone in stats_devs_gone:
           classification = self.recomender(dev_gone, devs_stats, k, decimal_places)
           self.save_dev_s_replacements(dev_gone, classification)

    def get_stats_devs_gone(self):
        devs_gone = self.read_developers_gone()
        stats_devs_gone = []

        for dev_gone in devs_gone:
            dev_stats = self.get_developer_stats(dev_gone)
            if not dev_stats:
                dev_stats = {'dev': dev_gone}
            
            stats_devs_gone.append(dev_stats)
        
        return stats_devs_gone
    
    def update_devs_stats(self, devs_gone_stats):
        devs_stats = copy.deepcopy(self.__devs_stats_cf_iff)
        for dev_gone_stats in devs_gone_stats:
            if dev_gone_stats in devs_stats:
                devs_stats.remove(dev_gone_stats)
